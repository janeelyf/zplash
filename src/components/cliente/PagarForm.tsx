          ...
          {resultado.encontrado ? (
            <>
              ...
              {!mostrarAuto && (
                ...
                >
                  {pagando === "renovacion" ? "Redirigiendo..." : `Renovar plan — ${fmtCLP(precios.plan.precio)}`}
                </button>
              )}
            </>
          ) : (
            <>
              ...
              {!mostrarAuto && (
                <button
                  ...
                  disabled={pagando !== null || accionPlan !== null}
                >
                  {pagando === "plan_nuevo" ? "Redirigiendo..." : `Contratar plan — ${fmtCLP(precios.plan.precio)}`}
                </button>
              )}
            </>
          )}
          ...
          {!soloPagoUnico &&
            (!mostrarAuto ? (
              <button className="btn ghost" style={{ marginTop: 10 }} onClick={() => setMostrarAuto(true)}>
                Renovación automática — {fmtCLP(precios.planOneclick.precio)}/mes (ahorras{" "}
                {fmtCLP(precios.plan.precio - precios.planOneclick.precio)})
              </button>
            ) : (
              <div className="field" style={{ marginTop: 14 }}>
                ...
                <button className="btn" style={{ marginTop: 10 }} onClick={activarAutomatica} disabled={inscribiendo}>
                  {inscribiendo ? "Redirigiendo..." : `Activar renovación automática — ${fmtCLP(precios.planOneclick.precio)}/mes`}
                </button>
              </div>
            ))}
        </div>
